...
$_SESSION["password"]=hash('sha512', $_POST["password"]); // con questo associo il parametro username che mi è stato passato dal form alla variabile SESSION password
$query = mysql_query("SELECT * FROM users WHERE username='".$_POST["username"]."' AND password ='".hash('sha512', $_POST["password"])."'")  //per selezionare nel db l'utente e pw che abbiamo appena scritto nel log
...
echo "non ti sei loggato con successo"; // altrimenti esce scritta a video questa stringa di errore
}
?>
